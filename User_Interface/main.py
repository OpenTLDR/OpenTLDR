...
from opentldr import KnowledgeGraph, log
...
async def get_all_reference_data():
    with KnowledgeGraph() as kg:
        ...
        return response_as_json('{ "nodes" : '+to_json_list(n,kg)+', "edges" : '+to_json_list(e,kg)+'}')
...
async def get_reference_node_by_uid(uid:Optional[str]):
    with KnowledgeGraph() as kg:
        ...
        if node is None:
            raise HTTPException(status_code=404, detail="Reference node ({id}) not found".format(id=uid))
...
async def add_user(new_reference_node:Payload_Reference_Node):
    ...
    with KnowledgeGraph() as kg:
        node=kg.add_reference_node(new_reference_node.text, new_reference_node.type, new_reference_node.hypothesized)
        return response_as_json(node.to_json(kg))
...
async def get_reference_edge_by_uid(uid:Optional[str]):
    with KnowledgeGraph() as kg:
        ...
        if edge is None:
            raise HTTPException(status_code=404, detail="Reference edge ({id}) not found".format(id=uid))
...
async def add_reference_edge(new_reference_edge:Payload_Reference_Edge):
    ...
    with KnowledgeGraph() as kg:
        ...
        if from_node is None:
            raise HTTPException(status_code=404, detail="Reference node ({id}) not found".format(id=new_reference_edge.from_uid))
    
        to_node = kg.get_reference_ndoe_by_uid(new_reference_edge.to_uid)
        if to_node is None:
            raise HTTPException(status_code=404, detail="Reference node ({id}) not found".format(id=new_reference_edge.to_uid))
    
        edge=kg.add_reference_edge(from_node, to_node, new_reference_edge.text, new_reference_edge.type, new_reference_edge.hypothesized)
        return response_as_json(edge.to_json(kg))
...
async def get_user_by_uid(user_uid:Optional[str]):
    with KnowledgeGraph() as kg:
        ...
        if user is None:
            raise HTTPException(status_code=404, detail="User ({id}) not found".format(id=user_uid))
...
async def get_request_by_uid(uid:str):
    with KnowledgeGraph() as kg:
        ...
        if r is None:
            raise HTTPException(status_code=404, detail="Request ({id}) not found".format(id=uid))
...
async def add_request(new_request:Payload_UserRequest):
    ...
    if not new_request.user_name or not new_request.user_email:
        raise HTTPException(status_code=403, detail="User information required for new Request")
    if not new_request.request_title or not new_request.request_text:
        raise HTTPException(status_code=403, detail="Request title and text required for new Request")
    ...
    with KnowledgeGraph() as kg:
        ...
        r:Request=kg.add_request(new_request.request_title,new_request.request_text,user)
        return response_as_json(r.to_json(kg))
...
async def get_tldrs_for_request_uid_and_date(request_uid:str, date:Optional[str]):
    ...
    with KnowledgeGraph() as kg:
        ...
        if date is not None:
            ...
            if tldr is None:
                raise HTTPException(status_code=404, detail="TLDR for Request ({id}) and Date ({date}) not found".format(id=request_uid, date=d))
            tldrs.append(tldr)
        else:
...
async def get_tldr_by_uid(uid:str):
    with KnowledgeGraph() as kg:
        ...
        if tldr is None:
            raise HTTPException(status_code=404, detail="TLDR ({id}) not found".format(id=uid))
...
async def root(http_request:Http_Request):
    ...
    with KnowledgeGraph() as kg:
        ...
        for request in requests:
            if user_email is not None:
                user:User = request.requested_by.single()
                if user.email == user_email or user.email == "digger@opentldr.org" or user.name == "public":
    ...
    return templates.TemplateResponse(
         request=http_request, name="main.html", context={"data": out, "auth": user_email}
    )
...
async def tldr_list_report(http_request:Http_Request, request_uid:str, date:Optional[str]):
    with KnowledgeGraph() as kg:
        d = None
        if date is not None:
        ...
        if tldr is None:
            raise HTTPException(status_code=404, detail="TLDR Report for Request ({id}) and Date ({date}) not found".format(id=request_uid, date=d))
...
async def tldr_report(http_request:Http_Request, uid: str):
    with KnowledgeGraph() as kg:
        ...
        if tldr is None:
            raise HTTPException(status_code=404, detail="TLDR Report for UID ({id}) not found".format(id=uid))
        tldr_json = produce_tldr(kg,tldr)
...
async def get_forwarding_to_content_link(tldr_entry_uid:str):
    with KnowledgeGraph() as kg:
        ...
        if tldr_entry is None:
            raise HTTPException(status_code=404, detail="TLDR Entry for ({id}) not found".format(id=tldr_entry_uid))
...
async def set_feedback_rating(feedback:Payload_Feedback):
    ...
    with KnowledgeGraph() as kg:
        ...
        if entry is None:
            ...
            return "Error, no TLDR Entry for uid ({uid}) for feedback.".format(uid=feedback.feedback_entry_id)
...
def produce_request_timeline(kg:KnowledgeGraph, request:Request, tldrs:list[Tldr]) -> dict:
    out:dict = dict()
...
async def timeline(http_request:Http_Request, uid: str):
    with KnowledgeGraph() as kg:
        ...
        if request is None:
            raise HTTPException(status_code=404, detail="Request for UID ({id}) not found".format(id=uid))
    ...
    return templates.TemplateResponse(
         request=http_request, name="timeline.html", context={"timeline_json": timeline_json, "request_title": request.title }
    )
...
async def digger_new(http_request:Http_Request):
    ...
    return templates.TemplateResponse(
         request=http_request, name="digger.html", context={"request_json":default_request_json}
    )
...
async def digger_existing(http_request:Http_Request, uid:str):
    ...
    with KnowledgeGraph() as kg:
        ...
        if request_node is None:
            raise HTTPException(status_code=404, detail="Request node ({id}) not found".format(id=uid))
        
        request_json= DiggerRequest.model_validate_json(request_node.to_json(kg))

        return templates.TemplateResponse (
            request=http_request, name="digger.html", context={"request_json":request_json.json()}
        )
...
async def digger_update_request(dr:DiggerRequest):
    ...
    with KnowledgeGraph() as kg:
        if dr.uid == "":
            ...
            if r is None:
                raise HTTPException(status_code=403, detail="Invalid Request UID updated")
            r.title=dr.title
            r.text=dr.text
            r.save()
            dr= DiggerRequest.model_validate_json(r.to_json(kg))
...
async def digger_forward_to_content_link(content_uid:str):
    with KnowledgeGraph() as kg:
        ...
        if c is None:
            raise HTTPException(status_code=404, detail="Content for ({id}) not found".format(id=content_uid))
